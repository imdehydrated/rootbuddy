...
export function actionExplanation(action: Action, state: GameState): string {
  switch (action.type) {
    ...
    case ACTION_TYPE.AID:
      return `Aid ${factionLabel(action.aid?.targetFaction)} in clearing ${action.aid?.clearingID ?? "?"} with ${describeKnownCardID(action.aid?.cardID ?? 0)} and item slot ${action.aid?.itemIndex ?? "?"}; ${aidItemTakeLabel(action, state)}.`;
